...
* Upgraded aws cli version 1 using pip3 (python)
...
* Command used to create the swarm manager: `docker-machine create -d amazonec2 --amazonec2-vpc-id $AWS_VPC_ID --amazonec2-instance-type t2.micro --amazonec2-subnet-id $AWS_VPC_SUBNET --amazonec2-security-group docker-swarm-sg docker-swarm-manager`. Same command is used to create worker instances.
...
* It is better to add --advertise-addr and --listen-addr, even though i didn't do it for worker nodes.
...
* A docker nodes availability depends on whether scheduler can assigns tasks. It doesn't check whether the node is down or not.